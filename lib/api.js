...
function shouldRetryWithTrailingSlash(path) {
...
export async function request(path, { method = "GET", body, headers, retry = true } = {}) {
  ...
  async function performRequest(targetPath, { allowRefresh = retry, allowSlashRetry = true } = {}) {
    ...
    if (response.status === 401 && allowRefresh) {
      try {
        ...
        finalHeaders.Authorization = `Bearer ${newTok}`;
        return performRequest(targetPath, { allowRefresh: false, allowSlashRetry });
      } catch (e) {
        clearSession();
        throw e;
      }
    ...
    if (response.status === 404 && allowSlashRetry && shouldRetryWithTrailingSlash(targetPath)) {
      return performRequest(ensureTrailingSlash(targetPath), { allowRefresh, allowSlashRetry: false });
    }
  ...
  return performRequest(normalizedPath);
}
